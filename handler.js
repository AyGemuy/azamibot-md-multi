  ...
  try {
    ...
    try {
      ...
      if (m.chat.endsWith("@g.us")) {
        ...
        if (typeof akinator !== "object") db.data.chats[m.chat].akinator = {};
        if (akinator) {
          if (!("sesi" in akinator)) akinator.sesi = false;
          if (!("server" in akinator)) akinator.server = null;
          if (!("frontaddr" in akinator)) akinator.frontaddr = null;
        ...
        if (typeof mutepinkey !== "object") db.data.chats[m.chat].mutepinkey = {};
        if (mutepinkey) {
          if (!("remoteJid" in mutepinkey)) mutepinkey.remoteJid = "";
          if (!("fromMe" in mutepinkey)) mutepinkey.fromMe = false;
          if (!("id" in mutepinkey)) mutepinkey.id = "";
      ...
      if (typeof selfpinkey !== "object") db.data.datas.selfpinkey = {};
      if (selfpinkey) {
        if (!("remoteJid" in selfpinkey)) selfpinkey.remoteJid = "";
        if (!("fromMe" in selfpinkey)) selfpinkey.fromMe = false;
        if (!("id" in selfpinkey)) selfpinkey.id = "";
    ...
    for (let name in plugins) {
      ...
      if (usedPrefix = (match[0] || "")[0]) {
        ...
        if (m.chat in db.data.chats || m.sender in db.data.users) {
          ...
          if (!/unbanchat/.test(m.plugin) && chat?.isBanned) return;
          if (!/unbanuser/.test(m.plugin) && user?.banned) return;
          if (!anti && !zzz && chat?.adminonly && !isAdminOwner) return;
          if (!isROwner && opts["self"] && !anti) return;
          if (opts["pconly"] && m.chat.endsWith("g.us") && !anti) return;
          if (opts["gconly"] && !m.chat.endsWith("g.us") && !isPrems && !/_|menfess|ww/.test(m.plugin)) return;
          if (!zzz && chat?.owneronly && !isOwner) return;
        }
...
export async function pollUpdate(message) {
  for (const {
    ...
    if (message.pollUpdates) {
      const pollCreation = this.loadMessage(key.remoteJid, key.id) || this.loadMessage(key.id);
      if (pollCreation) {
