...
export default async function(m, conn = {
  ...
  let img;
  try {
    if (Helper.opts["img"]) img = /sticker|image/gi.test(m.mtype) ? await terminalImage.buffer(await m.download()) : false;
  } catch (e) {
    console.error(e);
  }
...
`.trim(), me + " ~" + conn.user.name, (m.messageTimestamp ? new Date(1e3 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date()).toTimeString(), m.messageStubType ? WAMessageStubType[m.messageStubType] : "", filesize, filesize === 0 ? 0 : (filesize / 1009 ** Math.floor(Math.log(filesize) / Math.log(1e3))).toFixed(1), ["", ..."KMGTP"][Math.floor(Math.log(filesize) / Math.log(1e3))] || "", sender, m ? m.exp : "?", user ? "|" + user.exp + "|" + user.limit : "|", m.chat + (chat ? " ~" + chat : ""), m.mtype ? m.mtype.replace(/message$/i, "").replace("audio", m.msg.ptt ? "PTT" : "audio").replace(/^./, v => v.toUpperCase()) : "");
