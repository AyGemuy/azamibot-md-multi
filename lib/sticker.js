...
async function addExif(webpSticker, packname, author, categories = [], extra = {}) {
  ...
  const json = {
    ...
    emojis: categories,
    "link-group-bot": "https://cutt.ly/azamilaifu2",
    "link-script-github": "https://github.com/clicknetcafe/azamibot-md-multi",
    ...extra
  };
...
async function video2webp(media, fps = 15) {
  ...
  await new Promise((resolve, reject) => {
    ff(tmpFileIn).on("error", reject).on("end", () => resolve(true)).addOutputOptions(["-vcodec", "libwebp", "-vf", `scale='min(320,512)':min'(320,512)':force_original_aspect_ratio=decrease,fps=${fps}, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`, "-loop", "0", "-ss", "00:00:00", "-t", "00:00:05", "-preset", "default", "-an", "-vsync", "0"]).toFormat("webp").save(tmpFileOut);
  });
