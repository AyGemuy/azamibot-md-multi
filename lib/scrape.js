...
function aksaraToLatin(str, options = {
  HVokal: false
}) {
...
function latinToAksara(str, options = {
  mode: "ketik",
  space: true
}) {
  const aksara = new Aksara(str, options);
...
async function artimimpi(mimpi) {
  ...
  const results = (_e = (_d = (_c = (_b = (_a = data.split("</i></b><br><br>")[1]) === null || _a === void 0 ? void 0 : _a.split("\x3c!-- AWAL IN-ARTICLE ADV --\x3e")[0]) === null || _b === void 0 ? void 0 : _b.replace(/<(\/)?font( color=#ff0000)?>/gi, "")) === null || _c === void 0 ? void 0 : _c.trim()) === null || _d === void 0 ? void 0 : _d.split("<br><br>")) === null || _e === void 0 ? void 0 : _e.filter(v => v);
...
async function googleImage(query) {
  ...
  return [...matches].map(({
    groups
  }) => decodeUrl(groups === null || groups === void 0 ? void 0 : groups.url)).filter(v => /.*\.jpe?g|png$/gi.test(v)).map(value => GoogleImageSchema.parse(value));
}
...
async function neonimeSearch(query, pop = "ink", page = 1) {
  return new Promise(async resolve => {
    try {
      const {
        data
      } = await axios.get(`https://neonime.${pop}/page/${page}/?s=${query}`);
      const $ = cheerio.load(data);
      const result = [];
...
function pinterest(query) {
  return new Promise((resolve, reject) => {
    axios(`https://www.pinterest.com/resource/BaseSearchResource/get/?source_url=%2Fsearch%2Fpins%2F%3Fq%3D${query}&data=%7B%22options%22%3A%7B%22isPrefetch%22%3Afalse%2C%22query%22%3A%22${query}%22%2C%22scope%22%3A%22pins%22%2C%22no_fetch_context_on_resource%22%3Afalse%7D%2C%22context%22%3A%7B%7D%7D&_=1619980301559`).then(data => {
      resolve(data.data.resource_response.data.results.map(v => ({
        id: v.id,
        title: v.title,
...
class Primbon {
  ...
  async zodiak(zodiak) {
    return new Promise((resolve, reject) => {
      axios.get(`https://primbon.com/zodiak/${zodiak}.htm`).then(({
        data
      }) => {
        let $ = cheerio.load(data);
        let fetchText = $("#body").text();
  ...
  async shio(shio) {
    return new Promise((resolve, reject) => {
      axios.get(`https://primbon.com/shio/${shio}.htm`).then(({
        data
      }) => {
        let $ = cheerio.load(data);
        let fetchText = $("#body").text();
...
async function youtubeSearch(query) {
  ...
  for (const content of contents) {
    ...
    if (tag === "videoRenderer") {
      ...
      const description = ((_b = data.detailedMetadataSnippets) === null || _b === void 0 ? void 0 : _b.pop().snippetText.runs.map(({
        text
      }) => text).join("")) || "";
    ...
    if (tag === "channelRenderer") {
      ...
      const description = data.descriptionSnippet.runs.map(({
        text
      }) => text).join("");
      channel.push({
        channelId: channelId,
...
async function ytdl(link) {
  const {
    data
  } = await axios.post("https://www.yt1s.com/api/ajaxSearch/index", new URLSearchParams({
    q: link,
    vt: "home"
  }), {
    headers: headers
...
async function download(id, k) {
  const {
    data
  } = await axios.post("https://www.yt1s.com/api/ajaxConvert/convert", new URLSearchParams({
    vid: id,
    k: k
  }), {
    headers: headers
...
async function ytdl2(url) {
  return new Promise(async resolve => {
    try {
      if (!url.match(/https:\/\/(www\.)?youtube\.com\/watch\?v=.*/i) && !url.match(/https:\/\/(www\.)?youtube\.com\/shorts\/.*/i) && !url.match(/https:\/\/(www\.)?youtube\.com\/embed\/.*/i) && !url.match(/https:\/\/(www\.)?youtube\.com\/v\/.*/i) && !url.match(/https:\/\/youtu\.be\/.*/i) && !url.match(/https:\/\/(www\.)?youtube\.com\/playlist\?list=.*/i)) return resolve({
        status: false,
