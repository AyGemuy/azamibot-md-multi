...
export async function before(m, {
  ...
  if (chat.antiLink && isGroupLink && !isAdmin) {
    ...
    if (!m.fromMe && isBotAdmin) {
      ...
      if (!newsletter && !danger) await this.reply(m.chat, `@${(m.sender || "").replace(/@s\.whatsapp\.net/g, "")} *terdeteksi* mengirim Link ${m.text.includes("/channel/") ? "Channel" : danger ? "Berbahaya" : "Group"}!`, fkontak, {
        mentions: [m.sender]
      });
