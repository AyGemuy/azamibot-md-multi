...
export async function before(m) {
  ...
  if (m.messageStubType == 21) {
    ...
    await this.reply(m.chat, `${edtr} telah mengeluarkan @${m.messageStubParameters[0].split`@`[0]} dari grup.`, fkontak, {
      mentions: [m.sender, m.messageStubParameters[0]]
    });
  } else if (m.messageStubType == 29) {
    await this.reply(m.chat, `${edtr} telah menjadikan @${m.messageStubParameters[0].split`@`[0]} sebagai admin.`, fkontak, {
      mentions: [m.sender, m.messageStubParameters[0]]
    });
  } else if (m.messageStubType == 30) {
    await this.reply(m.chat, `${edtr} telah memberhentikan @${m.messageStubParameters[0].split`@`[0]} dari admin.`, fkontak, {
      mentions: [m.sender, m.messageStubParameters[0]]
    });
    ...
    try {
      ...
      await this.sendMsg(id, {
        image: {
          url: `https://api.tioo.eu.org/${add ? "welcome" : "goodbye"}?name=${name}&gcname=${namegc}&ppgc=${anu}&member=${meta.participants.length}&pp=${ana}&bg=${bg}`
        },
        caption: text,
        mentions: [user]
      }, {
