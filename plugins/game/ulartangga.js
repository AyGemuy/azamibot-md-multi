...
let handler = async (m, {
  ...
  const nazz_cmd = {
    ...
    join: () => {
      ...
      if (ut[m.chat].players[m.sender]) return conn.sendMsg(m.chat, {
    ...
    delete: () => {
      if (!ut[m.chat]) throw "Tidak ada sesi permainan di chat ini!";
      if (ut[m.chat].host !== m.sender && Date.now() - ut[m.chat].date < 3e5) throw "Anda tidak dapat menghapus sesi permainan, karena anda bukanlah host, anda dapat menghapus session setelah " + timeToFixed(3e5 - (Date.now() - ut[m.chat].date));
      if (ut[m.chat].host !== m.sender && ut[m.chat].status === "PLAYING" && Date.now() - ut[m.chat].date < 1e6) throw "Anda tidak dapat menghapus sesi permainan, karena anda bukanlah host dan permainan sedang berlangsung, anda dapat menghapus session setelah " + timeToFixed(1e6 - (Date.now() - ut[m.chat].date));
      delete ut[m.chat];
...
${Object.keys(ut[m.chat].players).map(v => "- @" + v.split("@")[0]).join("\n")}`,
    ...
    exit: async () => {
      ...
      if (!Object.keys(ut[m.chat].players).includes(ut[m.chat].host)) {
        ...
        await conn.sendMsg(m.chat, {
          text: `Di karenakan host keluar, kedudukan host akan di pindah ke @${host.split("@")[0]}`,
          mentions: [host]
        }, {
          quoted: m
        });
...
handler.help = ["ulartangga (exp+) (money+)"];
handler.tags = ["game"];
...
async function kocok(m, ut, conn) {
  ...
  let key = await conn.sendMsg(m.chat, {
    sticker: {
      url: `https://raw.githubusercontent.com/fgmods/fg-team/main/games/dados/${dadu}.webp`
    },
    packname: "Created by Naaazzzzz",
    author: "Naaazzzzz"
  }, {
    quoted: m
  });
  ...
  if (ut[m.chat].players[m.sender].langkah === 100) {
    ...
    db.data.users[m.sender].exp += 1e3;
    ...
    await conn.sendMsg(m.chat, {
      ...
      caption: `@${m.sender.split("@")[0]} Menang\n+1000 exp\n+5 limit\n+5 Koin`,
      mentions: [m.sender]
    }, {
      quoted: key
    });
