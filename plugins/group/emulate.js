...
let handler = async (m, {
  ...
  if (/welcome|leave|bye/.test(tx)) {
    ...
    try {
      ...
      await conn.sendFile(m.chat, pp, "", text, fkontak, false, {
        mentions: [m.sender]
      });
    } catch (e) {
      console.log(e);
      ...
      await conn.sendMsg(m.chat, {
        image: {
          url: `https://api.siputzx.my.id/api/canvas/${add ? "welcome" : "goodbye"}v1?username=${name}&guildName=${namegc}&memberCount=${participants?.length || "99"}&guildIcon=${anu}&avatar=${ana}&background=${bg}`
        },
        caption: text,
        mentions: [m.sender]
      }, {
        quoted: fkontak
      }).catch(_ => conn.reply(m.chat, text, fkontak, {
        mentions: [m.sender]
      }));
    }
  } else if (/f(kontak(bot)?|troli|vn|vid|textt)/.test(tx)) {
    await conn.reply(m.chat, `Halo @${m.sender.split("@")[0]}, ini simulasi ${tx}`, /troli/.test(tx) ? ftroli : /vn/.test(tx) ? fvn : /vid/.test(tx) ? fvid : /textt/.test(tx) ? ftextt : /bot/.test(tx) ? fkontakbot : fkontak, {
      mentions: [m.sender]
    });
