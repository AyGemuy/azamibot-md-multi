...
let handler = async (m, {
  ...
  let img = await conn.profilePictureUrl(m.chat, "image").catch(_ => "https://raw.githubusercontent.com/clicknetcafe/json-db/main/avalonbot/media/avatar_contact.jpg");
  for (let i of users) {
    ...
    for (let x of res) {
      if (x.status == "403") {
        ...
        await conn.reply(m.chat, `Couldn't add @${x.jid.split("@")[0]}, send invitation...`, m, {
          mentions: [x.jid]
        });
        ...
        await conn.reply(m.chat, `You couldn't add @${x.jid.split("@")[0]} because they left the group recently. Try again later.`, m, {
          mentions: [x.jid]
        });
      } else if (x.status == "409") {
        await conn.reply(m.chat, `@${x.jid.split("@")[0]} already in this group.`, m, {
          mentions: [x.jid]
        });
      } else {
