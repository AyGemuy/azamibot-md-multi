...
*Created :* ${new Date(x.creation * 1e3).toDateString()}
...
_last group name changed by ${x.subjectOwner ? "@" + x.subjectOwner.split("@")[0] : "Unknown"} on ${new Date(x.subjectTime * 1e3).toDateString()}_
...
*Demote :* ${sDemote}`;
    ...
    await conn.sendFile(m.chat, pp, "", `${isBotAdmin ? txt : `${txt.replaceAll("chat.whatsapp.com", "chat.whatsapp|com")}`}`, m, false, {
      mentions: y
    });
  } catch (e) {
    console.log(e);
