...
let handler = async (m, {
  ...
  for (let i of participants) {
    if (/admin/i.test(i.admin || "")) admin.push(i.id);
  }
  ...
  if (somematch(data, who) || who == conn.user.jid || db.data.datas.prems.some(v => v.user == who)) return m.reply(`Gaboleh gitu :v`);
  if (isOwner || isAdmin || isPrems) {
    ...
    await conn.reply(who, `User @${who.split("@")[0]} di *mute* selama ${total} menit.`, fliveLoc, {
      mentions: [who]
    });
  } else {
