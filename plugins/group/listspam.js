...
let handler = async (m, {
  ...
  let batch = [meiko, rio].map(v => v = {
    status: false,
    numbers: v.map(w => w + "@s.whatsapp.net")
  });
  ...
  if (args[0] == "reset") {
    ...
    for (let x of array) {
      if (somematch(differflat, x)) {
        ...
        u.spamcount = 0;
        await conn.reply(x, `User @${x.replace(/@s\.whatsapp\.net/g, "")} di *mute* selama ${total} menit.`, null, {
          mentions: [x]
        });
        await delay(ranNumb(1e3, 1500));
      }
    }
    for (let x of batch) {
      if (x.status) {
        ...
        for (let y of x.numbers) {
          ...
          u.bannedcd = cooldown * total;
          u.spamcount = 0;
          await conn.reply(y, `User @${y.replace(/@s\.whatsapp\.net/g, "")} di *mute* selama ${total} menit.`, fliveLoc, {
            mentions: [y]
          });
          await delay(ranNumb(1e3, 1500));
        }
    ...
    for (let x of Object.keys(user)) {
      if (!petinggi.includes(x) && user[x].spamcount > ia) {
        let pn = await PhoneNumber("+" + x.split("@")[0]).getNumber("international");
        y = user[x].spamcount;
        name = await conn.getName(x);
        await array.push({
          title: `*${name.replace(/\n/g, "").trim()}*${name != pn ? ` (${pn})` : ""}`,
          rowId: `${usedPrefix}diem ${(y + ranNumb(1, 10)) * 3} ${x}`,
          description: `spam count : ${y}`,
          user: x,
          count: y
        });
      }
