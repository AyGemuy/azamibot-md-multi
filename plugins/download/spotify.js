  ...
  if (isUrl(text)) {
    if (/\/track\//.test(text)) {
      try {
        ...
        await conn.sendFAudio(m.chat, {
          [/mp3/g.test(command) ? "document" : "audio"]: {
            url: ttl
          },
          mimetype: "audio/mpeg",
          fileName: `${res.title}.mp3`
        }, m, res.title, "https://telegra.ph/file/2e15408ac5e72fc90bc3f.jpg", text);
      } catch (e) {
        console.log(e);
        let res = await SpottyDL.getTrack(text);
        ...
        try {
          ...
          }, m, anu.title, anu.thumbnail, args);
        } catch (e) {
          console.log(e);
          try {
            ...
            await conn.sendFAudio(m.chat, {
              [/mp3/g.test(command) ? "document" : "audio"]: {
                url: await aud.download()
              },
              mimetype: "audio/mpeg",
              fileName: `${anu.result.title}.mp3`
            }, m, anu.result.title, "https://telegra.ph/file/2e15408ac5e72fc90bc3f.jpg", args);
          } catch (e) {
            console.log(e);
            m.reply(e.message);
          }
