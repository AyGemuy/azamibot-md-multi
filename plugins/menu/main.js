...
let handler = async (m, {
  ...
  try {
    ...
    let _text = [before.replace(": *%limit", `${isPrems ? ": *Infinity" : ": *%limit"}`), ...Object.keys(tags).map(tag => {
      return header.replace(/%category/g, tags[tag]) + "\n" + [...help.filter(menu => menu.tags && menu.tags.includes(tag) && menu.help).map(menu => {
        return menu.help.map(help => {
          return body.replace(/%cmd/g, menu.prefix ? help : "%p" + help).replace(/%islimit/g, menu.limit ? "(Limit)" : "").replace(/%isPremium/g, menu.premium ? "(Premium)" : "").trim();
        }).join("\n");
      }), footer].join("\n");
    })].join("\n");
