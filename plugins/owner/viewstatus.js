...
let handler = async (m, {
  ...
  if (args[1] == "all") {
    ...
    if (meh.length == 0) return m.reply(`[!] Failed, @${who.split("@")[0] || ""} bukan pengguna WhatsApp.`, null, {
      mentions: [who]
    });
  ...
  await conn.reply(m.chat, chat, m, men ? {
    mentions: [men]
  } : {});
  if (enable) await conn.reply(m.sender, sender, fkontak, men ? {
    mentions: [men]
  } : {});
